...
const signupController = async (req, res, next) => {
  try {
    ...
    const msg = {
      to: req.body.email,
      from: 'bogdanmyagkiy@gmail.com',
      subject: 'Email verification link',
      text: `Hello! please verify your email to end your registration!: ${link}`,
      html: `<strong>Hello! please verify your email to end your registration!: ${link}</strong>`,
    };
...
const loginController = async (req, res, next) => {
  try {
    ...
    if (!isVerificated) {
      return res
        .status(400)
        .json({ message: 'User not verificated! Check your email for verification link.' });
    }
...
const updateAvatarController = async (req, res, next) => {
  ...
  try {
    ...
    // await fs.rename(temporaryName, resultUpload); - альтернатива без Jimp и ресайза
